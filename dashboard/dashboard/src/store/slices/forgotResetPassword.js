...
export const forgotPassword = (email) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post("http://localhost:4000/api/v1/user/password/forget", { email }, {
        ...
        dispatch(
        forgotResetPassSlice.actions.forgotPasswordFailed(errorMessage)
        );
    }
...
export const resetPassword = (token, password, confirmPassword) => async (dispatch) => {
    dispatch(forgotResetPassSlice.actions.resetPasswordRequest());
     try {
        ...
        dispatch(
        forgotResetPassSlice.actions.resetPasswordFailed(error.response.data.message)
        );
    }
};
